...
```{r, eval=TRUE}
if (keras::is_keras_available() & reticulate::py_available()) {
    ggseqlogo::ggseqlogo(substr(GenProSeq::example_PTEN, 1, 20))
}
```
...
if (keras::is_keras_available() & reticulate::py_available()) {
    library(GenProSeq)
    label <- substr(GenProSeq::example_luxA, 3, 3)
    ...
    # CVAE
    VAE_result <- fit_VAE(prot_seq = GenProSeq::example_luxA,
                        label = label,
                        length_seq = length_seq,
                        embedding_dim = embedding_dim,
                        ...
                        intermediate_decoder_layers = list(layer_dense(units = 16),
                                                            layer_dense(units = 128)),
                        prot_seq_val = GenProSeq::example_luxA,
                        label_val = label,
                        epochs = epochs,
                        batch_size = batch_size,
                        use_generator = FALSE,
...
```{r, eval=TRUE}
if (keras::is_keras_available() & reticulate::py_available()) {
    ggseqlogo::ggseqlogo(substr(GenProSeq::example_luxA, 1, 20))
}
```
